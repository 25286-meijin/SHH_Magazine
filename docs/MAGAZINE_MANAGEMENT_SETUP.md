...
`/admin` 的「醫訊管理」提供既有與未來醫訊的新增、修改、立即發布、排程發布及下架。管理員可編輯首頁標題、首頁摘要、正式期號、正式發行日期、封面正式標題、單一門診時刻表頁次、接駁車頁次及是否設為最新一期。
...
它會以 Supabase Cron 每分鐘檢查一次到期醫訊。管理員輸入的排程日期與時間以 `Asia/Taipei` 解讀，資料庫保存 UTC 時間。這不需要 Vercel Cron 或額外的 Vercel 環境變數。
...
5. 選擇「立即發布」或設定台灣日期、時間的「排程發布」，再按「確認發布」。
6. 排程醫訊會留在「排程中」，成功後自動移至「各期醫訊管理」；失敗或逾時則保留並顯示狀態，等待下一輪重試。
7. 若設為最新一期，發布成功後首頁、`/latest/outpatient`、`/latest/shuttle` 與 QR 月份選項會從同一筆 metadata 自動更新。

排程中的醫訊不會出現在公開首頁、歷期醫訊、公開 issue 頁或 Reader。QR Code 管理可以提前為排程醫訊建立永久 QR；正式 `/q/` 在發布前不會寫入掃碼紀錄，發布後同一張 QR 才開始直接導頁及統計。

「預覽導入頁面」使用受管理員驗證保護的 `/admin/preview/issues/<期號>?page=<頁碼>`，不經 `/q/`、不建立 `entry_id`，且明確關閉 Reader tracking，因此不會寫入 `qr_events`。
