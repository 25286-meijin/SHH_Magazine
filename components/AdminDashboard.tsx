  ...
  return <main className="admin-shell"><div className="wrap">
    ...
    {mode === "issues" ? <IssueManager onIssuesChanged={setIssueOptions} /> : mode === "qr" ? <>
      <section className="admin-section"><p className="eyebrow">QR CODE MANAGEMENT</p><h2>QR Code 管理</h2><p className="section-description">依序選擇月份、輸入主題與頁碼、再選擇公播區域。</p>
        ...
        <div className="table-wrap panel qr-routes-table-wrap"><div className="qr-routes-scroll"><table><thead><tr><th>月份</th><th>醫訊主題</th><th>導入頁碼</th><th>公播區域</th><th>QR ID</th><th>狀態</th><th>下載／管理</th></tr></thead><tbody>{routes.map(route => <tr key={route.qr_id}><td>{route.topic.issue_id}</td><td>{route.topic.title}</td><td>{route.topic.page_number ?? "原目的地"}</td><td>{route.placement.name}</td><td><code>{route.qr_id}</code></td><td>{route.active ? "啟用" : "已停用"}</td><td>{route.active ? <><a className="download-link" href={`/api/admin/qr-routes/${route.qr_id}/image?format=png&download=1`}>PNG</a><a className="download-link" href={`/api/admin/qr-routes/${route.qr_id}/image?format=svg&download=1`}>SVG</a><a className="download-link" href={`/admin/preview/issues/${encodeURIComponent(route.topic.issue_id)}?page=${route.topic.page_number ?? 1}`} target="_blank" rel="noreferrer">預覽導入頁面</a><button className="danger-link" type="button" onClick={() => deactivateRoute(route.qr_id)}>停用 QR</button></> : "保留歷史紀錄"}</td></tr>)}{!routes.length && <tr><td colSpan={7}>尚未建立 QR Code。</td></tr>}</tbody></table></div></div>
      </section>
